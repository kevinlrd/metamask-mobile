/* eslint-disable @typescript-eslint/no-explicit-any, no-var, eqeqeq, @typescript-eslint/prefer-for-of, @typescript-eslint/no-use-before-define -- TODO: Replace the legacy blockies implementation with a typed module. */
/**
...
function write(buffer: Uint8Array, offs: number, ...values: string[]): void {
  let offset = offs;
...
const PNG = function (this: any, width: number, height: number, depth: number) {
  this.width = width;
  this.height = height;
  this.depth = depth;
  ...
  this.index = function (x: any, y: any) {
  ...
  this.color = function (red: any, green: any, blue: any, alpha: any) {
  ...
  this.fillRect = function (x: any, y: any, w: any, h: any, color: any) {
    for (let i = 0; i < w; i++) {
      for (let j = 0; j < h; j++) {
...
function toDataUrl(address: any) {
