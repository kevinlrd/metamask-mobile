/* eslint-disable @typescript-eslint/no-explicit-any -- TODO: Type legacy middleware test doubles. */
...
describe('middlewares', () => {
  ...
  describe('createOriginMiddleware', () => {
    it('appends origin to request', () => {
      ...
      expect((req as any).origin).toBe(origin);
    ...
    it('initializes params as empty array if not present', () => {
      ...
      const next = jest.fn();
      middleware(req, {}, next);
      expect((req as any).params).toEqual([]);
    });
